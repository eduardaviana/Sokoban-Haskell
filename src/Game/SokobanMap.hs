...
-- Tipo de dado que define a estrutura de um único nível no JSON.
-- 'tileMapLocal': O mapa do nível;
-- 'marksLocal': As posições das marcas;
-- 'deriving (Show, Generic)': 'Show' para exibição e 'Generic' para que Aeson possa decodificar automaticamente.
data LevelLocal = LevelLocal
  { tileMapLocal :: [String],
    marksLocal :: [[Int]]
...
-- | Função auxiliar para customizar a decodificação JSON. Posui como parametro:
-- | String: um sufixo 
-- | String: uma string
-- | Remove o sufixo da string se ele existir.
dropSuffix :: String -> String -> String
dropSuffix suffix str
  | suffix `isSuffixOf` str = take (length str - length suffix) str
  | otherwise = str

-- | Declara que 'LevelLocal' pode ser decodificado a partir de JSON.
